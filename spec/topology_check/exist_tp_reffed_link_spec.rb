RSpec.describe 'check existence of tp refered in link src/dst', :checkup do
  ...
  it 'found source tp ref is not exists' do
    ...
    expect(msg[:path]).to eq 'nw1/node1,X1,node2,p1'
    expect(msg[:message]).to eq 'link source path:nw1/node1/X1 is not found in link:nw1/node1,X1,node2,p1'
  end

  it 'found destination tp ref is not exists' do
    msg = @result[:messages][1]
    expect(msg[:path]).to eq 'nw1/node1,p1,node2,Y1'
    expect(msg[:message]).to eq 'link destination path:nw1/node2/Y1 is not found in link:nw1/node1,p1,node2,Y1'
  end
end
