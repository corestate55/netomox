...
describe 'network diff (nw list)', :diff, :network, :nw do
  context 'when nw list changed' do
    before do
      nw_link0_def = NWTopoDSL::Network.new('nwX')
      nw_link1_def = NWTopoDSL::Network.new('nwX') do
        bdlink %w[node1 tp1 node2 tp2]
      end
      nw_link2_def = NWTopoDSL::Network.new('nwX') do
        bdlink %w[node1 tp1 node2 tp2]
        bdlink %w[node3 tp1 node4 tp2]
      end
      nw_link2_changed_def = NWTopoDSL::Network.new('nwX') do
        bdlink %w[node1 tp1 node2 tp2]
        bdlink %w[node3xx tp1 node2 tp2]
      end
      @nw_link0 = TopoChecker::Network.new(nw_link0_def.topo_data)
      @nw_link1 = TopoChecker::Network.new(nw_link1_def.topo_data)
      @nw_link2 = TopoChecker::Network.new(nw_link2_def.topo_data)
      @nw_link2_changed = TopoChecker::Network.new(nw_link2_changed_def.topo_data)
    end
