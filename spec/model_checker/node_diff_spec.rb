...
describe 'node diff (supporting-node list)', :diff, :node do
  context 'when node literal attribute changed' do
    before do
      node_def = NWTopoDSL::Node.new('nodeX', '')
      @node = TopoChecker::Node.new(node_def.topo_data, 'nwX')
      @node_kept = TopoChecker::Node.new(node_def.topo_data, 'nwX')
      @node_changed = TopoChecker::Node.new(node_def.topo_data, 'nwY')
    end
  ...
  context 'when support node list changed', :support do
    before do
      node_sup0_def = NWTopoDSL::Node.new('nodeX', '')
      node_sup1_def = NWTopoDSL::Node.new('nodeX', '') do
        support %w[nw1 node1]
      end
      node_sup2_def = NWTopoDSL::Node.new('nodeX', '') do
        support %w[nw1 node1]
        support %w[nw1 node2]
      end
      node_sup2_changed_def = NWTopoDSL::Node.new('nodeX', '') do
        support %w[nw1 node1]
        support %w[nw1 node2aa]
      end

      @node_sup0 = TopoChecker::Node.new(node_sup0_def.topo_data, '')
      @node_sup1 = TopoChecker::Node.new(node_sup1_def.topo_data, '')
      @node_sup2 = TopoChecker::Node.new(node_sup2_def.topo_data, '')
      @node_sup2_changed = TopoChecker::Node.new(node_sup2_changed_def.topo_data, '')
    end
