...
describe 'termination point diff (supporting-tp list)', :diff, :tp do
  context 'when tp literal attribute changed' do
    before do
      tp_def = NWTopoDSL::TermPoint.new('tpX', '')
      @tp = TopoChecker::TerminationPoint.new(tp_def.topo_data, 'nodeX')
      @tp_kept = TopoChecker::TerminationPoint.new(tp_def.topo_data, 'nodeX')
      @tp_changed = TopoChecker::TerminationPoint.new(tp_def.topo_data, 'nodeY')
    end
  ...
  context 'when support tp list changed', :support do
    before do
      tp_sup0_def = NWTopoDSL::TermPoint.new('tpX', '')
      tp_sup1_def = NWTopoDSL::TermPoint.new('tpX', '') do
        support %w[foo bar hoge]
      end
      tp_sup2_def = NWTopoDSL::TermPoint.new('tpX', '') do
        support %w[foo bar baz]
        support %w[foo bar hoge]
      end
      tp_sup2_changed_def = NWTopoDSL::TermPoint.new('tpX', '') do
        support %w[foo bar baz]
        support %w[foo bar changed]
      end
      @tp_sup0 = TopoChecker::TerminationPoint.new(tp_sup0_def.topo_data, '')
      @tp_sup1 = TopoChecker::TerminationPoint.new(tp_sup1_def.topo_data, '')
      @tp_sup2 = TopoChecker::TerminationPoint.new(tp_sup2_def.topo_data, '')
      @tp_sup2_changed = TopoChecker::TerminationPoint.new(tp_sup2_changed_def.topo_data, '')
    end
