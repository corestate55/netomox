...
describe 'networks diff (network list)', :diff, :networks, :network do
  context 'when network literal attribute changed' do
    before do
      nws_nw0_def = NWTopoDSL::Networks.new
      nws_nw1_def = NWTopoDSL::Networks.new do
        network 'nwX'
      end
      nws_nw2_def = NWTopoDSL::Networks.new do
        network 'nwX'
        network 'nwY'
      end
      nws_nw2_changed_def = NWTopoDSL::Networks.new do
        network 'nwX'
        network 'nwYaa'
      end
      @nws_nw0 = TopoChecker::Networks.new(nws_nw0_def.topo_data)
      @nws_nw1 = TopoChecker::Networks.new(nws_nw1_def.topo_data)
      @nws_nw2 = TopoChecker::Networks.new(nws_nw2_def.topo_data)
      @nws_nw2_changed = TopoChecker::Networks.new(nws_nw2_changed_def.topo_data)
    end
