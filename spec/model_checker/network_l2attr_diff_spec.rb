...
describe 'network diff with L2 attribute', :diff, :network, :attr, :l2attr do
  before do
    nw_l2attr0_def = NWTopoDSL::Network.new('nwX') do
      type NWTopoDSL::NWTYPE_L2
    end
    nw_l2attr_def = NWTopoDSL::Network.new('nwX') do
      type NWTopoDSL::NWTYPE_L2
      attribute(name: 'nwX', flags: %w[foo bar baz])
    end
    nw_l2attr_changed_def = NWTopoDSL::Network.new('nwX') do
      type NWTopoDSL::NWTYPE_L2
      attribute(name: 'nwX', flags: %w[foo bar hoge])
    end

    @nw_l2attr0 = TopoChecker::Network.new(nw_l2attr0_def.topo_data)
    @nw_l2attr = TopoChecker::Network.new(nw_l2attr_def.topo_data)
    @nw_l2attr_changed = TopoChecker::Network.new(nw_l2attr_changed_def.topo_data)
  end
