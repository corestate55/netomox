...
describe 'link diff with L2 attribute', :diff, :link, :attr, :l2attr do
  before do
    ...
    link_l2attr0_def = NWTopoDSL::Link.new(*link_spec, l2nw_type)
    link_l2attr_def = NWTopoDSL::Link.new(*link_spec, l2nw_type) do
      attribute(link_attr)
    end
    link_l2attr_changed_def = NWTopoDSL::Link.new(*link_spec, l2nw_type) do
      attribute(link_attr_changed)
    end

    @link_l2attr0 = TopoChecker::Link.new(link_l2attr0_def.topo_data, '')
    @link_l2attr = TopoChecker::Link.new(link_l2attr_def.topo_data, '')
    @link_l2attr_changed = TopoChecker::Link.new(link_l2attr_changed_def.topo_data, '')
  end
