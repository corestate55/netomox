...
describe 'network diff (supporting-node list)', :diff, :network do
  context 'when network literal attribute changed' do
    before do
      l2nw_def = NWTopoDSL::Network.new('nwX') do
        type NWTopoDSL::NWTYPE_L2
      end
      l3nw_def = NWTopoDSL::Network.new('nwX') do
        type NWTopoDSL::NWTYPE_L3
      end
      @l2nw = TopoChecker::Network.new(l2nw_def.topo_data)
      @l3nw = TopoChecker::Network.new(l3nw_def.topo_data)
    end
  ...
  context 'when support network list changed', :support do
    before do
      nw_sup0_def = NWTopoDSL::Network.new('nwX')
      nw_sup1_def = NWTopoDSL::Network.new('nwX') do
        support 'nw1'
      end
      nw_sup2_def = NWTopoDSL::Network.new('nwX') do
        support 'nw1'
        support 'nw2'
      end
      nw_sup2_changed_def = NWTopoDSL::Network.new('nwX') do
        support 'nw1'
        support 'nw2aa'
      end

      @nw_sup0 = TopoChecker::Network.new(nw_sup0_def.topo_data)
      @nw_sup1 = TopoChecker::Network.new(nw_sup1_def.topo_data)
      @nw_sup2 = TopoChecker::Network.new(nw_sup2_def.topo_data)
      @nw_sup2_changed = TopoChecker::Network.new(nw_sup2_changed_def.topo_data)
    end
