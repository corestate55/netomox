RSpec.describe 'node dsl', :dsl, :node do
  ...
  it 'generate single node' do
    node = Netomox::DSL::Node.new('nodeX', '')
    node_data = {
      'node-id' => 'nodeX',
      @tp_key => []
    }
  ...
  it 'generate node that has supporting-node', :support do
    node = Netomox::DSL::Node.new('nodeX', '') do
      support %w[nw1 node1]
    end
    node_data = {
      'node-id' => 'nodeX',
  ...
  it 'generate node that has L2 attribute', :attr, :l2attr do
    ...
    node = Netomox::DSL::Node.new('nodeX', @l2nw_type) do
      attribute(node_attr)
    end
  ...
  it 'generate node that has L3 attribute', :attr, :l3attr do
    ...
    node = Netomox::DSL::Node.new('nodeX', @l3nw_type) do
      attribute(pref)
    end
    # TODO: default values are OK?
    node_data = {
